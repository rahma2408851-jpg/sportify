...
mongoose
  .connect(
    process.env.MONGO_URI || "mongodb://127.0.0.1:27017/sportify"
  )
...
if (!process.env.SESSION_SECRET && isProduction) {
  console.warn(
    "Warning: SESSION_SECRET is not set. Set it in production for secure sessions."
  );
}
...
app.get(
  "/api/products",
  productController.getStorefrontProducts
);
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(
      `Sportify server running on http://localhost:${PORT}`
    );
  });
}
